...
def test_single_point(parser):
    ...
    assert sec_run.section_basis_set_cell_dependent[
        0
    ].basis_set_planewave_cutoff.magnitude == approx(6.53961708e-16)
    assert (
        sec_run.section_basis_set_atom_centered[0].basis_set_atom_centered_short_name
        == "DZVP-GTH-PADE"
    )
    ...
    assert sec_scc.section_scf_iteration[
        1
    ].energy_total_scf_iteration.magnitude == approx(-1.35770357e-16)
...
def test_geometry_optimization(parser):
    ...
    assert sec_sccs[11].section_scf_iteration[
        -1
    ].energy_total_scf_iteration.magnitude == approx(-7.48333145e-17)
...
def test_molecular_dynamics(parser):
    ...
    assert sec_sccs[10].x_cp2k_section_md_step[
        0
    ].x_cp2k_md_kinetic_energy_instantaneous == approx(2.34172483e-20)
