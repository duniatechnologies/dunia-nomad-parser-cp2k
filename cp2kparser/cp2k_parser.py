...
class CP2KOutParser(TextParser):
    ...
    def init_quantities(self):
        ...
        scf_wavefunction_optimization_quantities = [
            ...
            Quantity(
                "atom_forces",
                r"ATOMIC FORCES in \[a\.u\.\]\s*.+([\s\S]+?)SUM",
                convert=False,
        ...
        single_point_quantities = [
            Quantity(
                "self_consistent",
                r"SCF WAVEFUNCTION OPTIMIZATION([\s\S]+?)\Z",
                repeats=False,
        ...
        molecular_dynamics_quantities = [
            ...
            Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        Quantity("ensemble_type", r"BLE TYPE\s*=\s*(.+)"),
                        Quantity("step_number", r"STEP NUMBER\s*=\s*(\d+)", dtype=int),
                        Quantity(
...
class CP2KParser(FairdiParser):
    ...
    def get_velocities(self, frame):
        ...
        try:
            return self.velocities_parser.trajectory[
                frame // self.velocities_parser._frequency
            ]
        except Exception:
    ...
    def parse_system(self, trajectory):
        ...
        if trajectory.positions is not None:
            ...
            sec_system.atom_positions = np.dot(
                trajectory.scaled_positions, lattice_vectors
            )
        ...
        sec_system.configuration_periodic_dimensions = [
            v in periodic for v in ("x", "y", "z")
        ]
    ...
    def parse_configurations_quickstep(self):
        ...
        def parse_quickstep_calculation(source):
            ...
            for key in ["energy_total", "atom_forces"]:
                ...
                sec_quickstep_calc.x_cp2k_quickstep_converged = (
                    source.get("converged") is not None
                )

            if source.get("electronic_kinetic_energy") is not None:
                sec_quickstep_calc.x_cp2k_electronic_kinetic_energy = source.get(
            ...
            if source.stress_tensor is not None:
                ...
                if source.stress_tensor_one_third_of_trace is not None:
                    ...
                    sec_stress.stress_tensor_one_third_of_trace = val.to(
                        "pascal"
                    ).magnitude
                ...
                if source.stress_eigenvalues_eigenvectors is not None:
                    ...
                    sec_stress.x_cp2k_stress_tensor_eigenvalues = (
                        val[0].to("pascal").magnitude
                    )
        ...
        def parse_calculations(calculations):
            ...
            sec_scc.single_configuration_to_calculation_method_ref = (
                sec_run.section_method[-1]
            )
        ...
        if geometry_optimization is not None:
            ...
            optimization_steps.extend(
                geometry_optimization.get("optimization_step", [])
            )
    ...
    def parse_method_quickstep(self):
        ...
        if atomic_kind_info is not None:
            sec_atom_kinds = sec_quickstep_settings.m_create(
                x_cp2k_section_atomic_kinds
            )
            for atom in atomic_kind_info.get("atom", []):
                ...
                sec_kind_basis_set = sec_atom_kind.m_create(
                    x_cp2k_section_kind_basis_set
                )
                ...
                sec_method_atom_kind.method_atom_kind_atom_number = (
                    self.get_atomic_number(atom.kind_label)
                )
        ...
        if total_maximum_numbers is not None:
            ...
            sec_maximum = sec_quickstep_settings.m_create(
                x_cp2k_section_maximum_angular_momentum
            )
        ...
        sec_method_basis_set.number_of_basis_sets_atom_centered = len(
            sec_run.section_basis_set_atom_centered
        )
    ...
    def parse_sampling_method(self):
        ...
        if self.sampling_method == "geometry_optimization":
            ...
            sec_geometry_opt = sec_sampling_method.m_create(
                x_cp2k_section_geometry_optimization
            )
            for step in optimization.get("optimization_step", []):
                ...
                if information is None:
                    continue
                sec_geometry_opt_step = sec_geometry_opt.m_create(
                    x_cp2k_section_geometry_optimization_step
                )
            ...
            geometry_change = (
                sec_geometry_opt_step.x_cp2k_optimization_step_size_convergence_limit
            )
            if geometry_change is not None:
                sec_sampling_method.geometry_optimization_geometry_change = (
                    geometry_change
                )
            threshold_force = (
            ...
            if threshold_force is not None:
                sec_sampling_method.geometry_optimization_threshold_force = (
                    threshold_force
                )
            ...
            for key, val in self.settings["md"].items():
                ...
                if key in [
                    ...
                    setattr(
                        sec_md_settings,
                        "x_cp2k_md_%s_print_frequency" % key,
                        int(val[0]),
                    )
    ...
    def parse_input(self):
        ...
        parse(
            "x_cp2k_section_input", self.inp_parser.tree, self.archive.section_run[-1]
        )
    ...
    def parse(self, filepath, archive, logger):
        ...
        sec_run.program_compilation_host = self.settings["cp2k"][
        ...
        if self.settings["cp2k"]:
            sec_program_information = sec_run.m_create(
                x_cp2k_section_program_information
            )
        ...
        for atom in atoms:
            ...
            if basis_set is not None:
                ...
                sec_basis_set.basis_set_atom_number = self.get_atomic_number(
                    atom.kind_label
                )
